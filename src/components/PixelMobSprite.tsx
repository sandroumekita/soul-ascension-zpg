...
// SVG rendering extracted outside component to avoid re-creation every render
const renderMobSvg = (icon: string, isBoss: boolean) => {
...
const PixelMobSpriteInner: React.FC<PixelMobSpriteProps> = ({
  icon,
  isBoss = false,
  size = 'md',
}) => {
  ...
  return (
    <div className={`relative inline-block select-none shrink-0 ${dimClass}`}>
      <div className="w-full h-full flex items-center justify-center relative">
        {renderMobSvg(icon, isBoss)}
      </div>
    </div>
  );
};

export const PixelMobSprite = React.memo(PixelMobSpriteInner);
