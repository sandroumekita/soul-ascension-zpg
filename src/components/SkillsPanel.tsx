  ...
  return (
    <div className="bg-slate-900/90 text-white p-4 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-xl">
      <h3 className="text-base font-bold flex items-center gap-2 text-purple-400">
        <Sparkles size={18} /> Coleção de Habilidades Espirituais
      </h3>
      ...
      <div className="grid grid-cols-2 gap-3 bg-black/40 p-3 rounded-lg border border-purple-500/30">
        ...
        <div className="bg-slate-950 p-2.5 rounded border border-purple-500/40">
          <div className="text-[10px] text-purple-400 font-semibold uppercase">{GAME_THEME.skillSlot1Label}</div>
        ...
        <div className="bg-slate-950 p-2.5 rounded border border-amber-500/40">
          <div className="text-[10px] text-amber-400 font-semibold uppercase">{GAME_THEME.skillSlot2Label}</div>
          <div className="text-xs font-bold text-white mt-1">
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {owned && (
                    <span className="text-[10px] px-1.5 py-0.2 bg-purple-950 text-purple-300 rounded font-mono border border-purple-500/40 font-bold">
                      Nv. {owned.level}
                    </span>
                  )}
                </div>
                <div className="text-xs text-gray-400 mt-0.5 max-w-xs">{skill.description}</div>
                <div className="text-[10px] text-gray-500 mt-1 flex gap-3 font-mono">
                  <span>Dano Efetivo: <strong className="text-purple-300">{(effectiveDmgMult * 100).toFixed(0)}%</strong> {level > 1 && <span className="text-emerald-400 text-[9px]">(+{((level - 1) * 15)}%)</span>}</span>
              ...
              {owned ? (
                <div className="flex gap-2">
                  ...
                  {skill.slotType === 2 && (
                    ...
                    >
                      {isEquippedSlot2 ? <Check size={12} /> : null} Slot 2
                    </button>
                  )}
                </div>
              ) : (
